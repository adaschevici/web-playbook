...
const chunkBy = (n) => (number) => {
  ...
  chunks = chunks.map((c, i) => {
    return { height: c, start: i * c };
  });

  const remainder =
    number - chunks[chunks.length - 1].start - chunks[chunks.length - 1].height;
  if (remainder > 0) {
    chunks.push({
      height: remainder,
      start: chunks[chunks.length - 1].start + chunks[chunks.length - 1].height,
    });
  }
